const studyAssistant = async (req, res) => {
  ...
  try {
    ...
    const answer = chatCompletion.choices[0]?.message?.content || "Sorry, I couldn't generate an answer right now.";

    res.status(200).json({ answer });
  } catch (error) {
    console.error("Error generating AI answer:", error);
    res.status(500).json({ message: "Failed to generate study assistant response." });
  }
};
